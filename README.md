...
> **ROI 退出提示**：如果配置了 `--tp-roi` 或 `--sl-roi`，机器人会根据主交易所的平均建仓价计算 ROI，在达到设定的止盈/止损百分比前不会进入下一步对冲平仓。
...
- `--tp-roi`: 可选的止盈 ROI 百分比（例如 `0.5` 表示 +0.5%）。配置后，机器人会在主交易所的平均建仓价达到该 ROI 时才进入对冲平仓步骤。
- `--sl-roi`: 可选的止损 ROI 百分比（填写正数）。配置后，机器人会在 ROI 下降到 `-sl-roi`% 时触发对冲平仓，限制每个对冲循环的最大回撤。
