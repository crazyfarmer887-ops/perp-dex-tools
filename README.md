...
- `--tp-roi`：相对于成交均价的止盈百分比。
- `--sl-roi`：相对于成交均价的止损百分比。
- `--tp-sl-order-type`：触发方式（`market` 或 `limit`，默认 `market`）。

设置这些参数后，机器人依然会在 ROI 止盈价挂出常规的限价平仓单；只有 BingX 会附带原生 TPSL 触发单，其它交易所会忽略这些触发器，并在日志中提示该功能暂不可用。
