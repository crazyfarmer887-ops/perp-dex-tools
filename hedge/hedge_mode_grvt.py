...
class HedgeBot:
    ...
    async def place_grvt_post_only_order(self, side: str, quantity: Decimal, target_price: Optional[Decimal] = None):
        ...
        self.grvt_order_status = None
        if target_price is not None:
            self.logger.info(f"[OPEN] [GRVT] [{side}] Placing GRVT ROI POST-ONLY order @ {target_price}")
        else:
        ...
        def order_update_handler(order_data):
            ...
            try:
                ...
                if status == 'FILLED' and self.grvt_order_status != 'FILLED':
                    ...
                    if side == 'buy' and self.grvt_position > 0:
                        self.long_entry_price = price_decimal
                        self.long_entry_size = self.grvt_position
                        self.logger.info(f"📌 Recorded long entry average price: {self.long_entry_price}")
                    elif side == 'sell' and self.grvt_position < 0:
                        self.short_entry_price = price_decimal
                        self.short_entry_size = abs(self.grvt_position)
        ...
        while iterations < self.iterations and not self.stop_flag:
            ...
            try:
                ...
                if self.pending_take_profit_price is not None:
                    self.logger.info(f"🎯 GRVT take-profit target ({side}): {self.pending_take_profit_price}")
                if self.pending_stop_loss_price is not None:
                    self.logger.info(f"🛡️ GRVT stop-loss threshold ({side}): {self.pending_stop_loss_price}")
                await self.place_grvt_post_only_order(side, self.order_quantity, target_price=self.pending_take_profit_price)
            except Exception as e:
                self.logger.error(f"⚠️ Error in trading loop: {e}")
                self.logger.error(f"⚠️ Full traceback: {traceback.format_exc()}")
                break
            ...
            try:
                ...
                if target_price is not None:
                    self.logger.info(f"🎯 GRVT take-profit target ({side}) for remainder: {target_price}")
                if stop_price is not None:
                    self.logger.info(f"🛡️ GRVT stop-loss threshold ({side}) for remainder: {stop_price}")
                await self.place_grvt_post_only_order(side, abs(self.grvt_position), target_price=target_price)
            except Exception as e:
                self.logger.error(f"⚠️ Error in trading loop: {e}")
                self.logger.error(f"⚠️ Full traceback: {traceback.format_exc()}")
                break
