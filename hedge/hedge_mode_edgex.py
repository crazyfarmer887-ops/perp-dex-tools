...
class HedgeBot:
    ...
    async def wait_for_roi(self) -> None:
        ...
        start_time = time.time()
        hundred = Decimal('100')
        self.logger.info("⏳ Waiting for ROI targets before placing closing edgeX order...")

        while not self.stop_flag:
            try:
                best_bid, best_ask = await self.fetch_edgex_bbo_prices()
            except Exception as exc:
                self.logger.warning(f"⚠️ Failed to fetch edgeX prices while waiting for ROI: {exc}")
