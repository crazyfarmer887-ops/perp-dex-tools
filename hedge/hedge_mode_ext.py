...
class HedgeBot:
    ...
    async def place_bbo_order(self, side: str, quantity: Decimal, target_price: Optional[Decimal] = None):
        ...
        if target_price is not None:
            ...
            order_result = await self.extended_client.place_close_order(
            ...
            adjusted_price = order_result.price

        if order_result.success:
            return order_result.order_id, adjusted_price
        else:
            raise Exception(f"Failed to place order: {order_result.error_message}")

    async def place_extended_post_only_order(self, side: str, quantity: Decimal, target_price: Optional[Decimal] = None):
        ...
        self.extended_order_status = None
        if target_price is not None:
            self.logger.info(f"[OPEN] [Extended] [{side}] Placing Extended ROI POST-ONLY order @ {target_price}")
        else:
            self.logger.info(f"[OPEN] [Extended] [{side}] Placing Extended POST-ONLY order")
        order_id, order_price = await self.place_bbo_order(side, quantity, target_price=target_price)
        ...
        def order_update_handler(order_data):
            ...
            try:
                ...
                if status == 'FILLED':
                    ...
                    if side == 'buy' and self.extended_position > 0:
                        self.long_entry_price = price_decimal
                        self.long_entry_size = self.extended_position
                        self.logger.info(f"📌 Recorded long entry average price: {self.long_entry_price}")
                    elif side == 'sell' and self.extended_position < 0:
                        self.short_entry_price = price_decimal
                        self.short_entry_size = abs(self.extended_position)
        ...
        while iterations < self.iterations and not self.stop_flag:
            ...
            try:
                ...
                if self.pending_take_profit_price is not None:
                    self.logger.info(f"🎯 Extended take-profit target ({side}): {self.pending_take_profit_price}")
                if self.pending_stop_loss_price is not None:
                    self.logger.info(f"🛡️ Extended stop-loss threshold ({side}): {self.pending_stop_loss_price}")
                await self.place_extended_post_only_order(side, self.order_quantity, target_price=self.pending_take_profit_price)
            except Exception as e:
                self.logger.error(f"⚠️ Error in trading loop: {e}")
                self.logger.error(f"⚠️ Full traceback: {traceback.format_exc()}")
                break
            ...
            try:
                ...
                if target_price is not None:
                    self.logger.info(f"🎯 Extended take-profit target ({side}) for remainder: {target_price}")
                if stop_price is not None:
                    self.logger.info(f"🛡️ Extended stop-loss threshold ({side}) for remainder: {stop_price}")
                await self.place_extended_post_only_order(side, abs(self.extended_position), target_price=target_price)
            except Exception as e:
                self.logger.error(f"⚠️ Error in trading loop: {e}")
                self.logger.error(f"⚠️ Full traceback: {traceback.format_exc()}")
                break
