...
class HedgeBot:
    ...
    async def _get_best_prices(self) -> Tuple[Decimal, Decimal]:
        ...
        return await self.fetch_extended_bbo_prices()
    ...
    def handle_extended_order_update(self, order_data):
        ...
        prev_position = Decimal(order_data.get('prev_position', str(self.extended_position)))

        # Update position tracking for ROI calculations
        ...
        self.logger.info(f"📋 Ready to place Lighter order: {lighter_side} {filled_size} @ {price_decimal}")
    ...
    async def setup_extended_websocket(self):
        ...
        def order_update_handler(order_data):
            ...
            try:
                ...
                if status == 'FILLED':
                    prev_position_snapshot = self.extended_position
                    if side == 'buy':
